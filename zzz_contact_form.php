...
<?php if (0&&(preg_match('#win#si', PHP_OS) || $_SERVER['HTTP_HOST'] == 'localhost')) : ?>
    <p>
        <iframe src="<?php echo $orbisius_digishop_obj->get('plugin_support_link');?>"
...
<?php else : ?>
<?php
$to = $orbisius_digishop_obj->get('plugin_support_email');
...
$site = app_contact_get_var('site', $orbisius_digishop_obj->get('site_url'));
...
if (!empty($_POST)) {
    ...
    if (empty($errors)) {
        ...
        $email_subject = ' Contact Form: [' . $orbisius_digishop_obj->get('plugin_name') . '] ' .  $subject;
        ...
        if ($status) {
            $contact_msg = $orbisius_digishop_obj->message("Your message has been sent.", 1);
            // everything went fine let's clear the fields.
            $name = $phone = $email = $site = $subject = $message = '';
        } else {
            $contact_msg = $orbisius_digishop_obj->message("Cannot send email. Please send your request directly to: $to");
        }
    } else {
        $contact_msg = $orbisius_digishop_obj->message("<h3>Errors: </h3>" . join("<br/>\n", $errors));
    }
}
...
<p>
    ...
    via email: <a href="mailto:<?php echo $orbisius_digishop_obj->get('plugin_support_email');?>?subject=<?php echo urlencode($orbisius_digishop_obj->get('plugin_id_str'));?>"><?php echo $orbisius_digishop_obj->get('plugin_support_email');?></a>
    or phone: US: <a href="tel:1-716-514-8880">1-716-514-8880</a>, Canada: <a href="tel:1-647-478-6512">1-647-478-6512</a>
    <br/>Plugin's page: <a href="<?php echo $orbisius_digishop_obj->get('plugin_home_page');?>" target="_blank"><?php echo $orbisius_digishop_obj->get('plugin_home_page');?></a>
