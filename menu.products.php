...
$inactive_product = str_replace( '%label%', 'Inactive', " <img src='$adm_prefix/images/product_inactive.png' title='%label%' alt='%label%' /> "); // . $orbisius_digishop_obj->m('Inactive')
$active_product = str_replace( '%label%', 'Active', " <img src='$adm_prefix/images/product_active.png' title='%label%' alt='%label%' /> "); //  . $orbisius_digishop_obj->m('Active', 1)
?>
...
<div class="orbisius_cyberstore">
    <div class="wrap">
        ...
        <p>The list of products you currently have. Copy the shortcode into the post where you'd like the buy now button to appear.</p>

        <div class="wrap" id="app-partners-container">
            <table class="widefat app_table_half0 wp-list-table posts">
                <thead>
                    <tr>
                            ...
                            <tr>
                                ...
                                <td><?php echo "[" . $orbisius_digishop_obj->get('plugin_id_str') . " id=\"{$rec['id']}\"]" ?>
                                    ...
                                    <?php if (!empty($rec['file']) && !empty($rec['hash'])) : ?>
                                        <small>
                                            <br/><a href="javascript:void(0);" onclick="jQuery('#download_link_container_<?php echo $rec['id']; ?>').toggle();">show/hide permanent download link</a>
                                            <div id="download_link_container_<?php echo $rec['id']; ?>" class="download_link app_hide widefat">
                                                ...
                                                ?>" onclick="this.select();" class="widefat"/>
